...
const Products = () => {
  ...
  const getAllProducts = async () => {
    try {
      // const { data } = await axios.get('http://localhost:2200/api/product/get-products');
      const { data } = await axios.get(`${host}/api/product/get-specific-products`);
      if (data?.success) {
           ...
           <div className="md-3 cd">
            {
              products.map(product => (
                <>
                <Link to={`/dashboard/admin/update-product/${product.slug}`}>
