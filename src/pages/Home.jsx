...
const Home = () => {
  ...
  const getAllProducts = async () => {
    ...
    const randomizedProducts = res.data.products.sort(() => Math.random() - 0.5);
  ...
  const loadMore = async () => {
    try {
      setLoading(true);
      const { data } = await axios.get(`${host}/api/product/product-list/${page}`);
      setLoading(false);
  ...
  const getFilteredProducts = async () => {
    try {
      ...
      const { data } = await axios.post(`${host}/api/product/product-filters`, filterArgs);
  ...
  const getCategoryFilteredProducts = async () => {
    try {
      ...
      const { data } = await axios.post(`${host}/api/product/product-category-filters`, filterArgs);
      setProducts(data?.products);
    } catch (err) {
      console.log(err);
    }
  ...
  const getPricesFilteredProducts = async () => {
    try {
      ...
      const { data } = await axios.post(`${host}/api/product/product-price-filters`, filterArgs);
      setProducts(data?.products);
    } catch (err) {
      console.log(err);
    }
            ...
            <div className="d-flex flex-column">
              {
                categories.map(category => (
                  <>
                    <Checkbox key={category._id} onChange={e => handleFilter(e.target.checked, category._id)} className='checkbox'>{category.name}</Checkbox>
                  </>
                ))
              }
            </div>
              ...
              <Radio.Group onChange={e => setRadio(e.target.value)}>
                {
                  Prices.map(price => (
                    <div key={price._id}>
                      <Radio value={price.array} className='radio'>{price.name}</Radio>
                    </div>
                  ))
                }
              </Radio.Group>
                    ...
                    <div className="card c" style={{ width: '15rem', height: '22rem' }}>
                      ...
                      <div className="card-body cb">
                        ...
                        <div className='d-flex buts'>
                          ...
                          <button href="#" className='btn btn-secondary but' 
                          onClick={()=> {
